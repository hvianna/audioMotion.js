...
<sub>Note: on Firefox you still can load local playlists and music files, if they're stored *inside audioMotion's folder* (tested up to Firefox version 66).</sub>
...
In the list below, the character inside brackets indicates the keyboard shortcut, when available.

+ **INFO** toggles the on-screen display of song information on every track change
+ **[S]CALE** toggles the display of the frequency scale on the X-axis; clicking the visualization area or pressing the **S** key also toggles this option
