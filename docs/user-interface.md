...
- **Settings** - open / close the [Settings panel](#settings-panel)
- **Config** - open the [Config](#config) window
- **Console** - open / close the [Console](#console) panel
...
You can save the current play queue by clicking the **Save as...** button.
...
🚧 On Firefox, Fill Opacity may not work properly with [RADIAL](#radial) mode, due to [this bug](https://bugzilla.mozilla.org/show_bug.cgi?id=1164912).
...
| switch | function |
|--------|----------|
| **AUTO** | select the **next gradient** on each track change, or **random gradient** on each mode change when [Random mode](#random-mode) is active |
...
| **STEREO** | toggle stereo (dual channel) analyzer, with separate graphs for left and right channels.<br>This setting does NOT change the audio output of stereo songs. |
