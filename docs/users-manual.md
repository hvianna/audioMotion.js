...
Moving the **Source** switch to the **AUX** position allows the visualization of any sounds generated in your computer,
including microphone input, system sounds and audio output by other programs.

When MIC/AUX is selected, the **Speakers** are automatically turned off to prevent feedback loops and/or distortions.
...
| **FLAT** | toggle outline (on) or shadow (off) for text messages displayed on canvas (shadowed mode may affect performance on some graphics cards) |
...
Any changes to the settings will be automatically saved and restored the next time you open audioMotion.
